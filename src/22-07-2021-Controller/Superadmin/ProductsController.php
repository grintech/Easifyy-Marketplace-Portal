...
class ProductsController extends BaseController
{
    ...
    public function pspServices()
    {
        $this->paginate = [
            'contain' => ['ProductTypes', 'ParentProducts', 'ProductGalleries','Merchants'],
            'conditions' => [
                    'Products.delete_status'=>1,
                    'Products.author >'=>1
                ],
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if($this->request->getData('_basic_price')<$this->request->getData('_basic_plan_price')){
                $this->Flash->error(__('Standard Original Price can\'t be smaller than the Plan Price.'));
                $this->set(compact('product','categories'));
                $this->render('add');
                return ;
            }
            if($this->request->getData('_standard_price')<$this->request->getData('_standard_plan_price')){
                $this->Flash->error(__('Pro Original Price can\'t be smaller than the Plan Price.'));
                $this->set(compact('product','categories'));
                $this->render('add');
                return ;
            }
            if($this->request->getData('_premium_price')<$this->request->getData('_premium_plan_price')){
                $this->Flash->error(__('Premium Original Price can\'t be smaller than the Plan Price.'));
                $this->set(compact('product','categories'));
                $this->render('add');
                return ;
            }
			...
			if ($product_id) {
                ...
                if($this->request->getData('reviewer_name')){
                    foreach($this->request->getData('reviewer_name') as $key =>$reviewer_names){
                        $reviewer_name_arr[] =  $reviewer_names;     
                    }
                    foreach($this->request->getData('review_text') as $key =>$review_texts){
                        $review_text_arr[] =  $review_texts;     
                    }
                    foreach($this->request->getData('rating') as $key =>$ratings){
                        $rating_arr[] =  $ratings;     
                    }
                    $this->saveProductReview($product_id,$reviewer_name_arr,$review_text_arr,$rating_arr,'ProductReviews');
                }
        ...
        $categories =  $this->Categories->ParentCategories->find('all', [ 
            ...
            'limit' => 200,
            'conditions'=>['delete_status'=>1,'status'=>1],
            ],
        );
        $this->set(compact('product','categories'));
    }
    ...
    public function edit($id = null)
    {
        ...
        $id = base64_decode($id);
        $product = $this->Products->get($id, [
            'contain' => ['ParentProducts', 'ProductCategories', 'ProductGalleries', 'ChildProducts','ProductPlans','ProductSellerPlans','ProductFaq','ProductReviews'],
        ]);
        //dd($product);
        $categories =  $this->Categories->ParentCategories->find('all', [ 
            'select' => ['id','name'],
            ...
            if ($product_id) {
              ...
              if( ($this->request->getData('product_type_id') == 4) ){
               ...
               return $this->redirect(['action' => 'edit', base64_encode($id) ]);  
                ...
                $this->Flash->error(__('The product could not be saved. Please, try again--2.'));
            }
            dd($product->getErrors());
            //print_r($product->getErrors());
            $this->Flash->error(__('The product could not be saved. Please, try again--1.'));
        }
        ...
        if($_standard_commission =='') {
            ...
            $product->_standard_commission= $_standard_commission;
        }
        ...
        if($_premium_commission =='') {
            ...
            $product->_premium_commission= $_premium_commission;
        }
        ...
        if($p_commssion_oprator =='') {
            ...
            $product->p_commssion_oprator=$p_commssion_oprator;
        }
        $p_commssion_add=$product->p_commssion_add;
        if($p_commssion_add =='') {
            $p_commssion_add=$parent_product_data->p_commssion_add;
            $product->p_commssion_add=$p_commssion_add;
        }

        
        if($product->_basic_gst==0) {
            $product->_basic_gst=$parent_product_data->_basic_gst;
        }
        if(!isset($product->_standard_gst)) {
            $product->_standard_gst=$parent_product_data->_standard_gst;
        }
        if(!isset($product->_premium_gst)) {
            $product->_premium_gst=$parent_product_data->_premium_gst;
        }

        if($product->_basic_gst_show==false) {
            $product->_basic_gst_show=$parent_product_data->_basic_gst_show;
        } 
        if($product->_standard_gst_show==false) {
            $product->_standard_gst_show=$parent_product_data->_standard_gst_show;
        }
        if($product->_premium_gst_show==false) {
            $product->_premium_gst_show=$parent_product_data->_premium_gst_show;
        }
        //dd($product);
        $this->set(compact('product','categories'));
        $this->render('add');
