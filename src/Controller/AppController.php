...
class AppController extends Controller
{
    ...
    protected function assignMerchant($order ) {
        ...
        echo $serviceID = $order->product_id;
    ...
    public function sendOrderEmails($order) {
        ...
        if($order->gross_total==$order->total) {
            $price = $order->gross_total;        
        } else {
            $price = $order->gross_total-$order->total;        
        }
    ...
    public function sendEmail($UserEmail,$msg,$type,$subject,$data){
       // if($data['order']->gross_total==$data['order']->total) {
        //     $price = $data['order']->gross_total;        
        // } else {
        //     $price = $data['order']->gross_total-$data['order']->total;
        //     $price = $data['order']->gross_total-$price;        
        // }
        ...
        $email
            ...
            ->setViewVars(
                        [
                            ...
                            'job_link' => BASE_URL.'service-details/'.$data['order']->product->slug,
