...
class BaseController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow( ['login','getcities']);
    }
        ...
        if( $model != "" ){
         ...
         if($product_id && $headings){
            ...
            foreach($headings as $key =>$heading){
                $productCategory_arr = [ $foreign_key => $product_id,'heading' => $heading,'price' => $prices[$key],'type' => $types[$key],'taxable' => $taxables[$key]]; 
                $productBrand = $this->{$model}->find()->where( $productCategory_arr)->first(); 
                    if($productBrand) {
                    //return true;
                    } else {
    ...
    public function getSubCate($id = null) {
        ...
        $subCategories = $this->Categories->find('list', array('conditions' => 'Categories.parent_id=' . $id,
                   'fields' => array('id', 'name')));
    ...
    */
     ...
     * @return Object User
     */
    public function __generateEmailVerificationToken($user) {
        if (empty($user)) {
            return null;
        }
        // Generate a random string 100 chars in length.
        ...
        $user['User']['email_verification_token'] = $hash;
        $user['User']['email_token_created_at'] = date('Y-m-d H:i:s');
        return $user;
    }
     ...
     * @return
     */
    public function __sendEmailVerificationEmail($id = null) {
        ...
        if (!empty($id)) {
            ...
            $User = $this->User->read();
            $reset_link = Router::url(array('controller' => 'Users', 'action' => 'validate_email_verification_token', $User['User']['email_verification_token']),true);

            $email = new CakeEmail();
            $email->emailFormat('html');
            $email->template('email-verification');
            $email->viewVars(array('reset_link' => $reset_link,'first_name' =>$User['User']['first_name']));
            $email->from('localstore@rndexperts.in');
            $email->to($User['User']['username']);
            $email->subject('Email Verificatioin Link');
            $email->send();
            return true;
        }
        return false;
