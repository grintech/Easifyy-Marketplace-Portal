...
<div class="couponRedeems view large-9 medium-8 columns content">
    ...
    <table class="vertical-table">
        ...
        <tr>
            ...
            <td><?= $couponRedeem->has('coupon') ? $this->Html->link($couponRedeem->coupon->id, ['controller' => 'Coupons', 'action' => 'view', $couponRedeem->coupon->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Order') ?></th>
            <td><?= $couponRedeem->has('order') ? $this->Html->link($couponRedeem->order->id, ['controller' => 'Orders', 'action' => 'view', $couponRedeem->order->id]) : '' ?></td>
        </tr>
        <tr>
